...
from codes.plot_curves import plot
import numpy as np
import pandas as pd
...
def model_eval_sentiment(dataloader, model, device, args, final):
    ...
    sentiment_acc = 100*accuracy_score(y_true, y_pred)
...
def model_eval_negation(neg_dataloader, model, device, args, final):
    ...
    with torch.no_grad():
        ...
        neg_accuracy = 100*np.mean(np.array(neg_y_pred) == np.array(neg_y_true))
        ...
        if final==True:
            ...
            cm = confusion_matrix(neg_y_true, neg_y_pred, labels=[0,1])
            disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=class_names)
            disp.plot() 
            plt.show()
...
def model_eval_sarcasm(sar_dataloader, model, device, args, final):
    ...
    with torch.no_grad():
        ...
        for batch in sar_dataloader:
            ...
            logits = model.predict_sarcasm(b_ids, b_mask)
            y_hat = logits.sigmoid().round().flatten().cpu().numpy()
            b_labels = b_labels.flatten().cpu().numpy()
        ...
        sarcasm_accuracy = 100*np.mean(np.array(sar_y_pred) == np.array(sar_y_true))
        
        if final==True:
            print(f"Final Sarcasm Accuracy {args.task} learning: {sarcasm_accuracy}")
            cm = confusion_matrix(sar_y_true, sar_y_pred)
            class_names = ['Not Sarcastic', 'Sarcastic']
            cm = confusion_matrix(sar_y_true, sar_y_pred, labels=[0,1])
            disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=class_names)
            disp.plot() 
            plt.show()
...
def model_eval_multitask(dataloaders, model, device, args, final=False):
    ...
    negation_accuracy = model_eval_negation(dataloaders['neg'], model, device, args, final)
    sarcasm_accuracy = model_eval_sarcasm(dataloaders['sar'], model, device, args, final)
    

    return (sentiment_accuracy, negation_accuracy, sarcasm_accuracy)
